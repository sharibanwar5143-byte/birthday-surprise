...
leftCake.className =
    "chocolate-cake cake-half cake-half-left";
...
rightCake.className =
    "chocolate-cake cake-half cake-half-right";
...
function goToBalloons() {
    ...
    if (!cake || !balloons) return;

    /* Prevent double transition */
    if (cake.dataset.transitioning === "true") return;

    cake.dataset.transitioning = "true";

    /* Hide hope page */
    if (hope) {
        hope.style.display = "none";
        hope.style.visibility = "hidden";
    }

    /* Prepare balloon page underneath cake */
    balloons.style.display = "flex";
    balloons.style.visibility = "visible";
    balloons.style.opacity = "0";
    balloons.style.transform = "scale(.96)";
    balloons.style.filter = "blur(8px)";
    balloons.style.zIndex = "501";

    /* Keep cake above balloons */
    cake.style.zIndex = "502";

    /* Start both pages together */
    requestAnimationFrame(function () {

        requestAnimationFrame(function () {

            /* CAKE → fade + slight zoom */
            ...
            cake.style.transform = "scale(1.04)";
            cake.style.filter = "blur(6px)";

            /* BALLOONS → smooth arrival */
            balloons.style.transition =
            ...
            balloons.style.filter = "blur(0)";
        });
    });

    /* Start balloon effects */
    setTimeout(function () {

        if (typeof startBalloonFireworks === "function") {
            startBalloonFireworks();
        }
      /* Start floating hearts */
    if (typeof startReasonHeartMagic === "function") {
        startReasonHeartMagic();
    }
    ...
    setTimeout(function () {
        ...
        balloons.style.transform = "scale(1)";
        balloons.style.filter = "blur(0)";
        balloons.style.zIndex = "999999";

        cake.dataset.transitioning = "false";
...
function goToFinalGift() {
    ...
    if (!letterScreen || !finalScreen) {
        return;
    }


    /* Put recipient name on final page */
    ...
    /* =========================================
       WAIT FOR LETTER TO FINISH
       ========================================= */

    setTimeout(function () {

        /* Completely remove old page */
      ...
      /* Start magical floating hearts */
startFinalHeartMagic();


        finalScreen.style.visibility =
            "visible";
...
/* =========================================================
   ONE LAST THING — CONTINUOUS HEART MAGIC
   ========================================================= */

let finalHeartTimer = null;


function startFinalHeartMagic() {
    ...
    if (finalHeartTimer) {
        clearInterval(finalHeartTimer);
    }


    /* Remove old generated particles */
    screen
        .querySelectorAll(
            ".final-floating-heart, .final-heart-sparkle"
        )
...
/* =========================================================
   5 REASONS — CONTINUOUS HEART MAGIC
   ========================================================= */

let reasonHeartTimer = null;
let reasonSparkleTimer = null;


function startReasonHeartMagic() {
    ...
    /* Create particle layer once */
    let layer =
        screen.querySelector(".reason-heart-layer");
    ...
    if (reasonHeartTimer) {
        clearInterval(reasonHeartTimer);
    }

    if (reasonSparkleTimer) {
        clearInterval(reasonSparkleTimer);
    }
    ...
    function createReasonHeart() {
        ...
        /* Only create while balloon page is visible */
        if (
