...
memoryCaption5: document.getElementById("memoryCaption5").value
  ...
  if (songFile) {
    birthdayData.song = await fileToDataURL(songFile);
}

for (let i = 1; i <= 5; i++) {
    const photoFile = document.getElementById("memoryPhoto" + i).files[0];

    if (photoFile) {
        birthdayData["memory" + i] = await fileToDataURL(photoFile);
    }
}
...
localStorage.setItem(
    ...
    document.getElementById("result").innerHTML = `
        ...
        <button onclick="previewSurprise()">
            Preview Surprise 🎁
        </button>
    `;
}
